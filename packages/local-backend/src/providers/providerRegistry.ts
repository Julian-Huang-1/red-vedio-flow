...
export class ProviderRegistry implements ProviderRegistryContract {
  ...
  private readonly fallbacks = new Map<Provider['modality'], Provider>()
  ...
  setFallback(provider: Provider) {
    this.fallbacks.set(provider.modality, provider)
  }

  resolve(providerId: string, modality: Provider['modality']) {
    const provider = this.providers.get(providerId) ?? this.fallbacks.get(modality)
    if (!provider) throw new Error(`provider not found: ${providerId}`)
    return provider
  }
}
